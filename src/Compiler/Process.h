...
private:
    ...
    // Minimal Win32-argv-compatible quoting: wrap in quotes and escape
    // embedded quotes. Sufficient for the plain paths/flags Pudl passes.
    ...
    static int runWindows(const std::vector<std::string> &args) {
        ...
        // shell string it re-parses/re-interprets) -- it just splits this
        // back into argv on the other side per Win32 quoting rules.
